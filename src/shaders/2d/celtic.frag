...
// Helper for Absolute value of DS number
vec2 ds_abs(vec2 a) {
...
vec3 solve(vec2 p) {
    ...
    for (int i = 0; i < 2000; i++) {
        ...
        // = (|x| + iy)^2 + c
        // = (|x|^2 - y^2) + i(2|x|y) + c
        // Note: |x|^2 == x^2 so real part is same as Mandelbrot!
        // Imaginary part uses |x|
        
        vec2 absZx = ds_abs(zx);
        
        vec2 two = ds_set(2.0);
        vec2 xy = ds_mul(absZx, zy); // Use ABS(zx) here
